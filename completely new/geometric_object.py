...
class rund(coil_geometry):
    ...
    def draw_coil(self):
        ...
        for i in range(num_windings):
            ...
            inner_circle = plt.Circle((x, y), self.cooling_radius, color='red', fill=False, linestyle='--')
            ax.add_patch(inner_circle)

            # Add self.winding_radius labels to the first circle
            if i == 0:
                ax.text(x + 0.1 * self.cooling_radius - self.winding_radius, y + 0.4 * self.winding_radius, 'winding_radius', fontsize=12, verticalalignment='center')
                ax.plot([x - self.winding_radius, x], [y, y], color='black', linewidth=1)
                ax.text(x + 0.1 * self.cooling_radius, y + self.winding_radius + 0.1 * self.cooling_radius, 'spacing_between_windings', fontsize=12, verticalalignment='center')
            ...
            if i == self.number_of_windings_x:
                ax.text(x - self.cooling_radius*1.1, y + 0.4*self.cooling_radius, 'cooling_radius', fontsize=12, verticalalignment='center')
        ...
        ax.text(-self.spacing_between_windings - 2 * self.cooling_radius, self.len_y/2-self.spacing_between_windings, 'len_y', fontsize=12, verticalalignment='center')
        ax.text(self.len_x/2 - self.spacing_between_windings - 0.4 * self.cooling_radius, self.len_y + 0.2 * self.cooling_radius, 'len_x', fontsize=12, verticalalignment='center')
        ax.add_patch(box)

        # Display important variables
        important_variables = {'winding_adius': self.winding_radius, 'cooling_radius': self.cooling_radius, 'spacing_between_windings': self.spacing_between_windings, 'length_x': wire_spacing*self.number_of_windings_x-self.spacing_between_windings, 'length_y': wire_spacing*self.number_of_windings_y-self.spacing_between_windings}
